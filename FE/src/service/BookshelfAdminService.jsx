...
const BookshelfAdminService = {
  ...
  async getUserLibrary(userId) {
    const res = HttpClient.get(`/admin/bookshelf/users/${userId}`);
    return res?.data;
  },

  // Add book to user's bookshelf with status: 'FAVORITE' | 'READING'
  async addToUserBookshelf(userId, bookId, status) {
    const res = HttpClient.post(`/admin/bookshelf/users/${userId}/books/${bookId}`, { status });
    return res;
  },

  // Remove book from user's bookshelf
  async removeFromUserBookshelf(userId, bookId, status) {
    const res = HttpClient.delete(`/admin/bookshelf/users/${userId}/books/${bookId}`, { params: { status } });
    return res;
  }
};
