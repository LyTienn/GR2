...
const AdminAuthorService = {
  ...
  async getAllAuthors(params = {}) {
    const res = HttpClient.get('/authors', { params });
    return res?.data || [];
  },

  // Lấy chi tiết tác giả theo ID
  async getAuthorById(id) {
    const res = HttpClient.get(`/authors/${id}`);
    return res?.data;
  },

  // Lấy sách của tác giả
  async getBooksByAuthor(id) {
    const res = HttpClient.get(`/authors/${id}/books`);
    return res?.data || [];
  },

  // Tạo tác giả mới (Admin only)
  async createAuthor(authorData) {
    const res = HttpClient.post('/authors', authorData);
    return res;
  },

  // Cập nhật tác giả (Admin only)
  async updateAuthor(id, authorData) {
    const res = HttpClient.put(`/authors/${id}`, authorData);
    return res;
  },

  // Xóa tác giả (Admin only)
  async deleteAuthor(id) {
    const res = HttpClient.delete(`/authors/${id}`);
    return res;
  }
};
