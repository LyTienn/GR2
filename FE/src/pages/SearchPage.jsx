...
const SearchPage = () => {
        ...
        const fetchFilters = async () => {
            try {
                const [subjectsRes, authorsRes] = await Promise.all([
                    firstValueFrom(HttpClient.get("/subjects", { search: { limit: 100 } })), 
    ...
    const fetchBooks = useCallback(async () => {
        try {
            ...
            setTotalPages(Math.ceil(total / 20)); // limit 20
    ...
    return (
        <div className='h-screen flex flex-col bg-slate-50 overflow-hidden'>
            ...
            <main className='w-full flex-1 overflow-hidden max-w-screen-2xl mx-auto px-8 pt-2 flex flex-col'>
                            ...
                            <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm mb-4 flex flex-col sm:flex-row justify-between items-center gap-4">
                                <div className="flex items-center gap-3 w-full sm:w-auto">
                                    <div className="flex items-center gap-2 text-sm text-slate-500 whitespace-nowrap">
                                        ...
                                        <span className="hidden sm:inline">{t("layout.searchpage.filterBar.title")}</span>
                                    </div>
                                    <select
                                        value={sortBy}
                                    ...
                                    >
                                        <option value="newest">{t("layout.searchpage.filterBar.option.newest")}</option>
                                        <option value="oldest">{t("layout.searchpage.filterBar.option.oldest")}</option>
                                        <option value="views">{t("layout.searchpage.filterBar.option.mostPopular")}</option>
                                        <option value="a-z">{t("layout.searchpage.filterBar.option.nameAZ")}</option>
                                        <option value="z-a">{t("layout.searchpage.filterBar.option.nameZA")}</option>
                                    </select>
                                </div>
                                ...
                                >
                                    <span>↺</span> {t("layout.searchpage.filterBar.resetBtn")}
                                </Button>
                            </div>
                            ...
                            {query && (
                                <div className="mb-4">
                                    <h1 className='text-2xl font-bold text-slate-800'>
                                        {t("layout.resultpage.restext")} <span className='text-transparent bg-clip-text bg-linear-to-r from-blue-600 to-indigo-600'>"{query}"</span>
                                    </h1>
                                    <p className='text-slate-500 mt-1'>
                                        {t("layout.resultpage.textleft")} {totalResults} {t("layout.resultpage.textright")}.
                                    </p>
                                </div>
                            )}
                                ...
                                <div className="flex flex-wrap gap-2 mb-4">
                                    {selectedType && (
                                        <Badge variant="secondary" className="px-3 py-1 bg-blue-50 text-blue-700 hover:bg-blue-100 flex items-center gap-1">
                                            {selectedType === 'FREE' ? t("layout.searchpage.sidebar.filterSection.bookTypeChoice.free") : t("layout.searchpage.sidebar.filterSection.bookTypeChoice.premium")}
                                    ...
                                    {selectedSubject && (
                                        <Badge variant="secondary" className="px-3 py-1 bg-indigo-50 text-indigo-700 hover:bg-indigo-100 flex items-center gap-1">
                                            {subjects.find(s => String(s.id) === selectedSubject)?.name || t("layout.searchpage.sidebar.filterSection.subject")}
                                    ...
                                    {selectedAuthor && (
                                        <Badge variant="secondary" className="px-3 py-1 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 flex items-center gap-1">
                                            {authors.find(a => String(a.id) === selectedAuthor)?.name || t("layout.searchpage.sidebar.filterSection.author")}
                                    ...
                                    <button onClick={clearFilters} className="text-xs text-slate-500 hover:text-red-500 underline ml-2">
                                        {t("layout.searchpage.sidebar.deleteFilterBtn")}
                                    </button>
                                </div>
                            )}
                        </div>
                            ...
                            {loading ? (
                                ...
                                <>
                                    {books.length > 0 ? (
                                        ...
                                        <div className="w-full flex-1 min-h-[400px] flex flex-col items-center justify-center py-20 bg-white rounded-2xl border border-dashed border-slate-200">
                                            <div className="bg-slate-50 p-6 rounded-full mb-4">
                                                <Search className="w-12 h-12 text-slate-300" />
                                            </div>
                                            <h3 className="text-xl font-bold text-slate-900">{t("layout.header.search_dropDown.noResult")}</h3>
                                            <p className="text-slate-500 max-w-md text-center mt-2 mb-6">
                                                {t("layout.resultpage.changeSearchText")}
                                            </p>
